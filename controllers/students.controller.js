...
const { STUDENT_PROFILE_TTL } = require("../config/cache");
...
exports.getStudentByRegNumber = async (req, res) => {
  try {
    ...
    const cacheKey = `student:${regNumber}`;
...
exports.updateStudent = async (req, res) => {
  try {
    ...
    await cache.del(`student:${student.regNumber}`);
  } catch (error) {
    res.status(400).json({ status: "error", message: error.message });
  }
};
...
exports.deleteStudentImage = async (req, res) => {
  try {
    ...
    await cache.del(`student:${regNumber}`);
  } catch (error) {
    res.status(400).json({
      status: "error",
      message: error.message,
