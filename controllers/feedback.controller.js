...
exports.createFeedbackData = async (req, res) => {
  try {
    const { date, questions, campus, campusCount } = req.body;
    
    // Validate required fields
    if (!date || !questions || !campus || !campusCount) {
      return res.status(400).json({
        status: "error",
        message: "All fields (date, questions, campus, campusCount) are required"
      });
    }
    ...
    if (invalidQuestions.length > 0) {
      return res.status(400).json({
        status: "error",
        message: `Invalid question numbers: ${invalidQuestions.map(q => q.questionNumber).join(", ")}`
      });
    }
    ...
    let feedbackData = await FeedbackData.findOneAndUpdate(
