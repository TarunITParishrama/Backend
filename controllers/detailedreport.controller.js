...
exports.createDetailedReport = async (req, res) => {
  try {
    ...
    const requiredFields = [
      ...
      "percentage", "percentile", "rank", "isPresent"
    ];
  ...
  try {
    ...
    if (markAbsentStudents && reportsToInsert.length > 0) {
      ...
      if (absentStudents.length > 0) {
        const absentResult = await DetailedReport.insertMany(absentStudents, { ordered: false });
...
exports.getDetailedReports = async (req, res) => {
  try {
    const { regNumber, testName, stream, campus, section, dateFrom, dateTo, isPresent } = req.query;
...
exports.markAbsentStudents = async (req, res) => {
  try {
    ...
    const test = await DetailedReport.findOne({ testName, stream, isPresent: true });
    if (!test) {
      return res.status(404).json({
        status: "error",
