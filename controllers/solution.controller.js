...
exports.getSolutionBank = async function (req, res) {
  try {
    ...
    const solutionBankEntries = await SolutionBank.find({
      ...
      // .limit(limit)
      .select(
        "solutionRef questionNumber correctOptions correctSolution isGrace date"
      );
    res.status(200).json({
      status: "success",
      total,
...
exports.updateSolutionById = async function (req, res) {
  try {
    ...
    if (!updatedSolution) {
      return res
        .status(404)
        .json({ status: "error", message: "Solution not found" });
    }
...
exports.deleteSolutionById = async function (req, res) {
  try {
    ...
    res
      ...
      .json({
        status: "success",
        message: "Solution and associated entries deleted",
      });
  } catch (err) {
    res.status(400).json({ status: "error", message: err.message });
  }
...
exports.updateSolutionBankById = async function (req, res) {
  try {
    ...
    if (!entry) {
      return res
        .status(404)
        .json({ status: "error", message: "SolutionBank entry not found" });
    }
...
exports.deleteSolutionBankById = async function (req, res) {
  try {
    const deletedEntry = await SolutionBank.findByIdAndDelete(
      req.params.entryId
    );
